...
func (c *CLI) addAuthCommands() {
	...
	loginCmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return c.authLogin(cmd.Context(), profile, loginClientID, loginClientSecret, loginFlow, !loginNoBrowser)
		},
	}
	...
	loginCmd.Flags().BoolVar(&loginNoBrowser, "no-browser", false, "Do not auto-open browser for authentication")
	...
	initCmd.Flags().BoolVar(&loginNoBrowser, "no-browser", false, "Do not auto-open browser for authentication")
...
func (c *CLI) authLogin(ctx context.Context, profile, clientID, clientSecret, flow string, openBrowser bool) error {
	if flow != "" && flow != "device" {
	...
	creds, err := c.authMgr.AuthenticateWithDeviceCode(ctx, clientID, clientSecret, scopes, c.stderr, openBrowser)
	if err != nil {
