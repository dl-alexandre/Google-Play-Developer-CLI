...
func (c *CLI) permissionsUsersList(ctx context.Context, developerID string, pageSize int64, pageToken string, all bool) error {
	...
	if strings.EqualFold(c.outputFormat, string(output.FormatTable)) {
		if err := c.renderPermissionsUsersTable(allUsers); err != nil {
			return c.OutputError(errors.NewAPIError(errors.CodeGeneralError,
				"failed to render users table: "+err.Error()))
		}
		return nil
	}
	return c.Output(result.WithServices("androidpublisher"))
...
func (c *CLI) renderPermissionsUsersTable(allUsers []interface{}) error {
	table := tablewriter.NewWriter(c.stdout)
	table.Header([]string{"email", "accessState", "expirationTime", "partial", "permissions"})
	...
	for _, item := range allUsers {
		...
		if err := table.Append([]string{
			...
			stringValue(user["accessState"], "-"),
			stringValue(user["expirationTime"], "-"),
			stringValue(user["partial"], "false"),
			permissionListString(user["developerAccountPermissions"]),
		}); err != nil {
...
func permissionListString(v interface{}) string {
	items, ok := v.([]string)
	if ok {
		if len(items) == 0 {
			return "-"
