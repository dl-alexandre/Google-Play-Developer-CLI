...
func (f *DeviceCodeFlow) RequestDeviceCode(ctx context.Context) (*DeviceCodeResponse, error) {
	...
	// #nosec G704 -- URL is hardcoded Google OAuth endpoint, not user-controlled
...
func displayDeviceCodePrompt(w io.Writer, resp *DeviceCodeResponse, openBrowser bool) {
	if w == nil || resp == nil {
		return
	}
	...
	urlToOpen := resp.VerificationURLComplete
	if urlToOpen == "" {
		urlToOpen = verificationURL
	}
	...
	if openBrowser && !isHeadless() {
		if err := browser.OpenURL(urlToOpen); err == nil {
			browserOpened = true
			_, _ = fmt.Fprintln(w, "Opening browser for authentication...")
		}
	}

	if browserOpened {
		_, _ = fmt.Fprintln(w, "If the browser didn't open, visit:", verificationURL)
		_, _ = fmt.Fprintln(w, "And enter code:", resp.UserCode)
	} else {
