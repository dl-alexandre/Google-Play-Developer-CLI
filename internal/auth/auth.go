...
func (m *Manager) Authenticate(ctx context.Context, keyPath string) (*Credentials, error) {
	...
	if gacPath != "" {
		if err := validatePath(gacPath); err != nil {
			return nil, errors.NewAPIError(errors.CodeValidationError,
				fmt.Sprintf("invalid GOOGLE_APPLICATION_CREDENTIALS path: %v", err)).
				WithHint("Use an absolute path without directory traversal")
		}
	...
	if err != nil {
		...
		if gacPath != "" {
			// #nosec G703 -- Path already validated above
...
func (m *Manager) AuthenticateWithDeviceCode(ctx context.Context, clientID, clientSecret string, scopes []string, promptWriter io.Writer, openBrowser bool) (*Credentials, error) {
	...
	flow := NewDeviceCodeFlow(oauthConfig, WithBrowserOpen(openBrowser))
	...
	displayDeviceCodePrompt(promptWriter, deviceResp, openBrowser)
