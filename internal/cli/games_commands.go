...
func (c *CLI) gamesServiceError(err error) *errors.APIError {
...
func (c *CLI) gamesOutput(data interface{}) error {
	result := output.NewResult(data)
	return c.Output(result.WithServices("gamesmanagement"))
}

func (c *CLI) gamesOutputError(err error) error {
	return c.Output(output.NewErrorResult(c.gamesServiceError(err)).WithServices("gamesmanagement"))
}

func (c *CLI) gamesAchievementsReset(ctx context.Context, achievementID string) error {
	svc, err := c.getGamesManagementService(ctx)
	if err != nil {
		return c.gamesOutputError(err)
	}
	...
	return c.gamesOutput(map[string]interface{}{
		"success":        true,
