...
func (c *CLI) appsList(ctx context.Context, pageSize int64, pageToken string, all bool) error {
	...
	if c.outputFormat == string(output.FormatTable) {
		if err := c.renderAppsTable(apps); err != nil {
			result := output.NewErrorResult(errors.NewAPIError(errors.CodeGeneralError,
				"failed to render apps table: "+err.Error())).WithServices("playdeveloperreporting")
			return c.Output(result)
		}
		return nil
	}
...
func (c *CLI) renderAppsTable(apps []interface{}) error {
	table := tablewriter.NewWriter(c.stdout)
	table.Header([]string{"package", "title", "resource"})

	for _, app := range apps {
		if err := table.Append([]string{
			firstNonEmpty(jsonPath(app, "packageName"), jsonPath(app, "appInfo.packageName"), "-"),
			firstNonEmpty(jsonPath(app, "title"), jsonPath(app, "displayName"), "-"),
			firstNonEmpty(jsonPath(app, "name"), "-"),
		}); err != nil {
