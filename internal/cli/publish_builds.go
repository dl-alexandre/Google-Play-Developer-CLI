...
func (c *CLI) publishBuildsList(ctx context.Context, buildKind, editID string) error {
	...
	if kind == buildTypeAll || kind == buildTypeBundle {
		bundleResp, err := publisher.Edits.Bundles.List(c.packageName, activeEditID).Context(ctx).Do()
		if err != nil {
			return c.OutputError(errors.NewAPIError(errors.CodeGeneralError, err.Error()))
		}
		bundles = bundleResp.Bundles
	}
	if kind == buildTypeAll || kind == buildTypeAPK {
		apkResp, err := publisher.Edits.Apks.List(c.packageName, activeEditID).Context(ctx).Do()
		if err != nil {
			return c.OutputError(errors.NewAPIError(errors.CodeGeneralError, err.Error()))
		}
		apks = apkResp.Apks
	...
	if strings.EqualFold(c.outputFormat, string(output.FormatTable)) {
		if err := c.renderBuildsTable(bundles, apks); err != nil {
			return c.OutputError(errors.NewAPIError(errors.CodeGeneralError,
				fmt.Sprintf("failed to render builds table: %v", err)))
		}
		return nil
	}
...
func (c *CLI) renderBuildsTable(bundles []*androidpublisher.Bundle, apks []*androidpublisher.Apk) error {
	table := tablewriter.NewWriter(c.stdout)
	table.Header([]string{"type", "versionCode", "sha1", "sha256"})

	for _, bundle := range bundles {
		if err := table.Append([]string{
			"bundle",
			jsonPath(bundle, "versionCode"),
			firstNonEmpty(jsonPath(bundle, "binary.sha1"), jsonPath(bundle, "sha1"), "-"),
			firstNonEmpty(jsonPath(bundle, "binary.sha256"), jsonPath(bundle, "sha256"), "-"),
		}); err != nil {
	...
	for _, apk := range apks {
		if err := table.Append([]string{
			"apk",
			jsonPath(apk, "versionCode"),
			firstNonEmpty(jsonPath(apk, "binary.sha1"), jsonPath(apk, "sha1"), "-"),
			firstNonEmpty(jsonPath(apk, "binary.sha256"), jsonPath(apk, "sha256"), "-"),
		}); err != nil {
			return errors.NewAPIError(errors.CodeGeneralError, fmt.Sprintf("failed to append table row: %v", err))
		}
	}
...
func jsonPath(v interface{}, path string) string {
	if v == nil || path == "" {
		return ""
	}
