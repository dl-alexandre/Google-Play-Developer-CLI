...
for automating Android app publishing and management tasks.`,
	...
	pf.StringVar(&c.storeTokens, "store-tokens", storeTokensAuto, "Token storage: auto, never, secure")
...
func (c *CLI) setup(_ *cobra.Command) error {
	...
	if envStore := config.GetEnvStoreTokens(); envStore != "" && c.storeTokens == storeTokensAuto {
		c.storeTokens = envStore
	}
	if c.storeTokens == storeTokensAuto && c.config.StoreTokens != "" {
...
func (c *CLI) applyEnvOverridesWithFF() {
	fs := flag.NewFlagSet("gpd-env", flag.ContinueOnError)
	fs.SetOutput(io.Discard)

	packageName := fs.String("package", "", "")
	outputFormat := fs.String("output", "", "")
	pretty := fs.Bool("pretty", false, "")
	timeout := fs.Duration("timeout", 0, "")
	storeTokens := fs.String("store-tokens", "", "")
	fields := fs.String("fields", "", "")
	quiet := fs.Bool("quiet", false, "")
	verbose := fs.Bool("verbose", false, "")
	keyPath := fs.String("key", "", "")
	authProfile := fs.String("auth-profile", "", "")

	if err := ff.Parse(fs, []string{}, ff.WithEnvVarPrefix("GPD")); err != nil {
		return
	}
	...
	if c.outputFormat == "json" && *outputFormat != "" {
		c.outputFormat = *outputFormat
	}
	if !c.pretty && *pretty {
		c.pretty = true
	}
	if c.timeout == 30*time.Second && *timeout > 0 {
		c.timeout = *timeout
	}
	if c.storeTokens == storeTokensAuto && *storeTokens != "" {
		c.storeTokens = *storeTokens
	}
	if c.fields == "" && *fields != "" {
		c.fields = *fields
	}
	...
	if c.profile == "" && *authProfile != "" {
		c.profile = *authProfile
	}
}
