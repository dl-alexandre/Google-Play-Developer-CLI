...
### [ASC Auth Parity Guide](auth-parity-guide.md)

Migration-focused authentication guide for teams moving from App Store Connect workflows to `gpd` auth patterns.

**Includes:**
- Auth decision tree (service account vs OAuth device flow)
- ASC-to-gpd command mapping
- Profile strategy for multi-environment usage
- `invalid_grant` troubleshooting and testing-mode token limits

### [ASC Workflow Mapping](asc-workflow-mapping.md)

Workflow mapping for ASC release, build, analytics, and localization tasks to `gpd` commands.

**Includes:**
- Submit/release decision paths (rollout/halt/rollback/promote)
- App/build model differences and Play-native alternatives
- Analytics/reporting scope boundaries
- Side-by-side metadata/localization task mappings
