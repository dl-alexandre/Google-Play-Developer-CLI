...
func TestListProfilesSingle(t *testing.T) {
	tempHome := t.TempDir()
	setConfigEnv(t, tempHome)
	...
	if profiles[0].Profile != "default" {
		t.Fatalf("expected default profile, got %q", profiles[0].Profile)
	}
}
...
func TestListProfilesMultiple(t *testing.T) {
	...
	if err := writeTokenMetadata("key1", meta1); err != nil {
		t.Fatalf("writeTokenMetadata error: %v", err)
	}
	if err := writeTokenMetadata("key2", meta2); err != nil {
		t.Fatalf("writeTokenMetadata error: %v", err)
	}
	if err := writeTokenMetadata("key3", meta3); err != nil {
		t.Fatalf("writeTokenMetadata error: %v", err)
	}
	...
	if profiles[0].Profile != "dev" || profiles[1].Profile != "prod" || profiles[2].Profile != "staging" {
		t.Fatalf("expected sorted profiles, got %v", profiles)
	}
}

func TestListProfilesDuplicateProfileLatestWins(t *testing.T) {
	tempHome := t.TempDir()
	setConfigEnv(t, tempHome)
	...
	if profiles[0].ClientIDHash != "hash2" {
		t.Fatalf("expected latest metadata (hash2), got %q", profiles[0].ClientIDHash)
	}
}
...
func TestListProfilesMissingDir(t *testing.T) {
	...
	if len(profiles) != 0 {
		t.Fatalf("expected empty profiles for missing dir, got %d", len(profiles))
	}
}

func TestWriteTokenMetadataCreateDir(t *testing.T) {
	tempHome := t.TempDir()
	setConfigEnv(t, tempHome)
...
func TestReadTokenMetadataInvalid(t *testing.T) {
	tempHome := t.TempDir()
	setConfigEnv(t, tempHome)

	path := tokenMetadataPath("invalid")
	_, err := readTokenMetadata(path)
	if err == nil {
		t.Fatalf("expected error for missing file")
	}
}

func TestFindTokenMetadataMultipleProfiles(t *testing.T) {
	tempHome := t.TempDir()
	setConfigEnv(t, tempHome)
	...
	found, key, err := findTokenMetadata("prod")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if found == nil || found.Profile != "prod" {
		t.Fatalf("expected prod profile")
	}
	if key != "key1" {
		t.Fatalf("expected key1, got %q", key)
	}
}

func TestLoadStoredTokenInvalidJSON(t *testing.T) {
	storage := &memoryStorage{available: true}
	m := NewManager(storage)
	m.SetStoreTokens("auto")

	if err := storage.Store("key", []byte("invalid json")); err != nil {
		t.Fatalf("store error: %v", err)
	}
	loaded, err := m.loadStoredToken("key")
	if err == nil {
...
func TestLoadStoredTokenNotFound(t *testing.T) {
	storage := &memoryStorage{available: true}
	m := NewManager(storage)
	m.SetStoreTokens("auto")

	loaded, err := m.loadStoredToken("nonexistent")
	if err == nil {
...
func TestLoadStoredTokenExpiredNoRefresh(t *testing.T) {
	storage := &memoryStorage{available: true}
	m := NewManager(storage)
	m.SetStoreTokens("auto")
